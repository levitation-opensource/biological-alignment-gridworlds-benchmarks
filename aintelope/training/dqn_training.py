...
class Trainer:
    ...
    def __init__(self, params, n_observations, action_space):
        ...
        self.optimizer = optim.AdamW(
            DQN(self.n_observations, self.action_space.n).parameters(), lr=self.hparams.lr, amsgrad=True
    ...
    def get_action(self, 
        ...
        if np.random.random() < epsilon:
            ...
            logger.debug("debug state", type(observation)) #TODO figure out when obs becomes state
    ...
    def optimize_models(self, step):
        for agent_id in self.policy_nets.keys():
            ...
            state_batch = torch.cat(batch.state) #torch.tensor(observation, dtype=torch.float32, device=device).unsqueeze(0) if bug
            action_batch = torch.cat(batch.action)
            reward_batch = torch.cat(batch.reward) #torch.tensor([reward], device=device) if bug
            ...
            for key in policy_net_state_dict:
                target_net_state_dict[key] = policy_net_state_dict[
                    key
                ] * self.hparams.tau + target_net_state_dict[key] * (1 - self.hparams.tau)
